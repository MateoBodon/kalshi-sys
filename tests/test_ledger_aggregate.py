...
def test_ledger_aggregate_rejects_wrong_schema(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.chdir(tmp_path)
    reports_dir = Path("reports/_artifacts")
    reports_dir.mkdir(parents=True, exist_ok=True)
    csv_path = reports_dir / "bad_ledger.csv"
    csv_path.write_text(
        "series,event,market,bin,side,price,model_p,market_p,delta_p,size,expected_contracts,expected_fills,fill_ratio,slippage_mode,impact_cap,fees_maker,ev_after_fees,pnl_simulated,timestamp_et,manifest_path,ledger_schema_version\n"
        "CPI,E1,M1,270.0,YES,0.45,0.55,0.40,0.15,10,10,1.0,top,0.02,0.12,0.23,0.23,2025-01-01T08:00:00-05:00,,2\n",
        encoding="utf-8",
    )
