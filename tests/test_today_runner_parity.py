...
def _daily_stub_factory(tmp_path: Path, go_flag: bool, recorded: list[list[str]]) -> callable:
    ...
    def _stub(argv: list[str]) -> None:
        ...
        (artifacts_dir / "latest_manifest.txt").write_text((tmp_path / "data" / "raw" / "manifest.json").as_posix(), encoding="utf-8")

    return _stub


def test_today_runner_passes_flags_and_prints_manifest(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_today_runner_exits_on_no_go(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    recorded: list[list[str]] = []
