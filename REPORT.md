...
- **Replay & parity:** new `python -m tools.replay --families SPX,NDX --date YYYY-MM-DD --hours 10,11,12,13,14,15,16 --epsilon 0.15 --out reports/_artifacts/` CLI replays T−5m..T−2s maker sessions, writes `replay_ev.parquet`, JSON summaries, and plots. `scripts/parity_gate.py` now enforces ΔEV per window type (hourly vs close) and ships stricter JSON monitors that `report.digest` renders in the “Replay Parity” section (worst bins included).
...
## Runtime Monitors
<!-- monitors:start -->
_Generated 2025-12-23 06:56:22 UTC_

- ⬜ ev_gap: NO_DATA
- ⬜ fill_vs_alpha: NO_DATA
- ⬜ ev_seq_guard: NO_DATA
- ⚠️ freeze_window: ALERT — Freeze active for CPI
- ✅ drawdown: OK
